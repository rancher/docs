...
> **Note:** If you are using a Private CA signed certificate, add `--set privateCA=true` and see [Adding TLS Secrets - Using a Private CA Signed Certificate]({{< baseurl >}}/rancher/v2.x/en/installation/resources-for-ha/tls-secrets/#using-a-private-ca-signed-certificate) to add the CA cert for Rancher.
