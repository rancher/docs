...
This guide walks you through create of a custom cluster that includes 3 nodes: a Linux node, which serves as a Kubernetes control plane node; another Linux node, which serves as a Kubernetes worker used to support DNS and Ingress for the cluster; and a Windows node, which is assigned the Kubernetes worker role and runs your Windows containers.

>**Notes:**
>
>- Windows worker adding script must run on Windows Server 2019 (core version 1809 or above) hosts. Core version 1803 and earlier versions do not properly support.
...
>- Containers built for core version 1803 or earlier cannot run on Windows Server 2019 as *Windows Server Container* directly, but might be supported by *Hyper-V Isolation*. For more information, see [Windows Container Types](https://docs.microsoft.com/en-us/virtualization/windowscontainers/about/#windows-container-types) and [Kubernetes next for Hyper-V Isolation](https://kubernetes.io/docs/setup/windows/#hyper-v-isolation).
>- Overlay (alpha) networking requires [KB4489899](https://support.microsoft.com/en-us/help/4489899) hostfix.
...
Node 2  | Linux (Ubuntu Server 16.04 recommended)           | [Worker]({{< baseurl >}}/rancher/v2.x/en/cluster-provisioning/#worker-nodes)
...
- For **L2bridge** networking, it's best to use the same Layer 2 network for all nodes. Otherwise, you need to configure the route rules for them.
- For **Overlay** (alpha) netwokring, you must confirm the Windows Server 2019 host with *KB4489899* hotfix.
- To support [DNS](https://kubernetes.io/docs/concepts/services-networking/dns-pod-service/) and [Ingress](https://kubernetes.io/docs/concepts/services-networking/ingress/), your cluster must include at least one Linux node dedicated to the Worker role.
...
If you're going to use **L2bridge** networking and hosting your nodes on any of the cloud services listed below, you must disable the private IP address checks for both your Linux or Windows hosts on startup. To disable this check for each node, follow the directions provided by each service below.
...
Since v1.14, Kubernetes announced joining Windows nodes to a Linux-based cluster, so you should select **Kubernetes Version** to `v1.14 or above`.

### Networking Option

And then choosing a **Network Provider** for a cluster that supports Windows, the only option available is **Flannel**:

![Flannel]({{< baseurl >}}/img/rancher/flannel.png)

#### Enable the Windows Support Option

After selecting network provider, you need to **Enable** the **Windows Support**:

![Enable Windows Support]({{< baseurl >}}/img/rancher/enable-windows-support.png)

#### Choose the Flannel Backend

For now, you could choose to use [**Host Gateway (L2bridge)**](https://github.com/coreos/flannel/blob/master/Documentation/backends.md#host-gw) or [**VXLAN (Overlay)**](https://github.com/coreos/flannel/blob/master/Documentation/backends.md#vxlan):

![Choose Windows Networking]({{< baseurl >}}/img/rancher/choose-windows-networking.png)

>**Notes:**
	Overlay networking is still on [Alpha](https://kubernetes.io/docs/setup/windows/#networking).
...
Node Roles            | etcd <br/> Control Plane <br/> Worker (recommend)
...
## 4. Add Linux Host for DNS and Ingress Support

After the initial provisioning of your custom cluster, your cluster only has a single Linux host. Add another Linux host, which will be used to support *DNS* and *Ingress* for your cluster.
...
## 6. Cloud-hosted VM Routes Configuration (Optional)
