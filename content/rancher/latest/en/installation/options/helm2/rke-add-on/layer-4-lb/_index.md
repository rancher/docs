...
>
>If you are currently using the RKE add-on install method, see [Migrating from a High-availability Kubernetes install with an RKE add-on]({{<baseurl>}}/rancher/latest/en/upgrades/upgrades/migrating-from-rke-add-on/) for details on how to move to using the Helm chart.
...
Provision three Linux hosts according to our [Requirements]({{<baseurl>}}/rancher/latest/en/installation/requirements).

## 2. Configure Load Balancer

We will be using NGINX as our Layer 4 Load Balancer (TCP). NGINX will forward all connections to one of your Rancher nodes. If you want to use Amazon NLB, you can skip this step and use [Amazon NLB configuration]({{<baseurl>}}/rancher/latest/en/installation/k8s-install-server-install/nlb/)
...
1. Download one of following templates, depending on the SSL certificate you're using.
	...
	- [Template for self-signed certificate<br/> `3-node-certificate.yml`]({{<baseurl>}}/rancher/latest/en/installation/options/cluster-yml-templates/3-node-certificate)
	- [Template for certificate signed by recognized CA<br/> `3-node-certificate-recognizedca.yml`]({{<baseurl>}}/rancher/latest/en/installation/options/cluster-yml-templates/3-node-certificate-recognizedca)

    >**Advanced Config Options:**
    >
    >- Want records of all transactions with the Rancher API? Enable the [API Auditing]({{<baseurl>}}/rancher/latest/en/installation/api-auditing) feature by editing your RKE config file. For more information, see how to enable it in [your RKE config file]({{<baseurl>}}/rancher/latest/en/installation/options/helm2/rke-add-on/api-auditing/).
...
- Create a backup of your Rancher Server in case of a disaster scenario: [High Availability Back Up and Restoration]({{<baseurl>}}/rancher/latest/en/installation/backups-and-restoration/ha-backup-and-restoration).
- Create a Kubernetes cluster: [Provisioning Kubernetes Clusters]({{<baseurl>}}/rancher/latest/en/cluster-provisioning/).
