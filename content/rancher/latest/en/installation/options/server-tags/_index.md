...
weight: 230
aliases:
 - /rancher/latest/en/installation/server-tags
---

This section describes how to choose a Rancher version.

For a high-availability installation of Rancher, which is recommended for production, the Rancher server is installed using a **Helm chart** on a Kubernetes cluster. Refer to the [Helm version requirements]({{<baseurl>}}/rancher/latest/en/installation/options/helm-version) to choose a version of Helm to install Rancher.
...
When installing, upgrading, or rolling back Rancher Server when it is [installed on a Kubernetes cluster]({{<baseurl>}}/rancher/latest/en/installation/k8s-install/), Rancher server is installed using a Helm chart on a Kubernetes cluster. Therefore, as you prepare to install or upgrade a high availability Rancher configuration, you must add a Helm chart repository that contains the charts for installing Rancher.
...
4. Continue to follow the steps to [upgrade Rancher]({{<baseurl>}}/rancher/latest/en/upgrades/upgrades/ha-server-upgrade-helm/) from the new Helm chart repository.
{{% /tab %}}
{{% tab "Docker Images" %}}
When performing [Docker installs]({{<baseurl>}}/rancher/latest/en/installation/single-node), upgrades, or rollbacks, you can use _tags_ to install a specific version of Rancher.
