...
weight: 251
aliases:
  - /rancher/latest/en/installation/proxy-configuration/
  - /rancher/latest/en/installation/single-node/proxy
---
...
Passing environment variables to the Rancher container can be done using `-e KEY=VALUE` or `--env KEY=VALUE`. Required values for `NO_PROXY` in a [Docker Installation]({{<baseurl>}}/rancher/latest/en/installation/single-node-install/) are:
