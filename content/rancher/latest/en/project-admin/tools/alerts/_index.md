...
Before you can receive alerts, one or more [notifier]({{<baseurl>}}/rancher/latest/en/cluster-admin/tools/notifiers) must be configured at the cluster level.

Only [administrators]({{<baseurl>}}/rancher/latest/en/admin-settings/rbac/global-permissions/), [cluster owners or members]({{<baseurl>}}/rancher/latest/en/admin-settings/rbac/cluster-project-roles/#cluster-roles), or [project owners]({{<baseurl>}}/rancher/latest/en/admin-settings/rbac/cluster-project-roles/#project-roles) can manage project alerts.
...
The scope for alerts can be set at either the [cluster level]({{<baseurl>}}/rancher/latest/en/cluster-admin/tools/alerts/) or project level.
...
When you enable monitoring for the project, some project-level alerts are provided. You can receive these alerts if a [notifier]({{<baseurl>}}/rancher/latest/en/cluster-admin/tools/notifiers) for them is configured at the cluster level.
...
For information on other default alerts, refer to the section on [cluster-level alerts.]({{<baseurl>}}/rancher/latest/en/cluster-admin/tools/alerts/default-alerts)
...
If you enable [project monitoring]({{<baseurl>}}/rancher/latest/en/project-admin/tools/#monitoring), this alert type monitors for the overload from Prometheus expression querying.
...
1. Input or select an **Expression**, the drop down shows the original metrics from Prometheus, including:
  ...
  - [**Customize**]({{<baseurl>}}/rancher/latest/en/project-admin/tools/monitoring/#project-metrics)
...
1. Finally, choose the [notifiers]({{<baseurl>}}/rancher/latest/en/cluster-admin/tools/notifiers/) that send you alerts.
