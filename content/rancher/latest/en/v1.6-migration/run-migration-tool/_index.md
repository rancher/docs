...
1. Export the Docker Compose files for your Cattle environments and stacks from Rancher v1.6.
    ...
    **Step Result:** migration-tools exports Compose files (`docker-compose.yml` and `rancher-compose.yml`) for each stack in the `--export-dir` directory. If you omitted this option, Compose files are output to your current directory.
    ...
    A unique directory is created for each environment and stack. For example, if we export each [environment/stack]({{<baseurl>}}/rancher/latest/en/v1.6-migration/#migration-example-files) from Rancher v1.6, the following directory structure is created:
...
>**Want full usage and options for the migration-tools CLI?** See the [Migration Tools CLI Reference]({{<baseurl>}}/rancher/latest/en/v1.6-migration/run-migration-tool/migration-tools-ref/).
...
If we parse the two example files from [Migration Example Files]({{<baseurl>}}/rancher/latest/en/v1.6-migration/#migration-example-files), `docker-compose.yml` and `rancher-compose.yml`, the following files are output:
...
>**Prerequisite:** [Install Rancher CLI]({{<baseurl>}}/rancher/latest/en/cli/) for Rancher v2.x.
...
[1]:{{<baseurl>}}/rancher/latest/en/v1.6-migration/monitor-apps/#configuring-probes-in-rancher-v2-x
[2]:{{<baseurl>}}/rancher/latest/en/v1.6-migration/schedule-workloads/#scheduling-using-labels
[3]:{{<baseurl>}}/rancher/latest/en/v1.6-migration/discover-services
[4]:{{<baseurl>}}/rancher/latest/en/v1.6-migration/expose-services
[5]:{{<baseurl>}}/rancher/latest/en/v1.6-migration/schedule-workloads/#scheduling-pods-to-a-specific-node

<!-- MB: oops, skipped 6 -->

[7]:{{<baseurl>}}/rancher/latest/en/v1.6-migration/schedule-workloads/#scheduling-using-labels
[8]:{{<baseurl>}}/rancher/latest/en/v1.6-migration/schedule-workloads/#scheduling-global-services
[9]:{{<baseurl>}}/rancher/latest/en/v1.6-migration/schedule-workloads/#label-affinity-antiaffinity

### [Next: Expose Your Services]({{<baseurl>}}/rancher/latest/en/v1.6-migration/expose-services/)
