...
weight: 3060
aliases:
  - /rancher/latest/en/tasks/projects/add-ssl-certificates/
---
...
1. Select the **Scope** of the certificate.
    ...
    - **Available to a single namespace:** The certificate is only available for the deployments in one [namespace]({{<baseurl>}}/rancher/latest/en/k8s-in-rancher/projects-and-namespaces/#namespaces). If you choose this option, select a **Namespace** from the drop-down list or click **Add to a new namespace** to add the certificate to a namespace you create on the fly.
...
Now you can add the certificate when launching an ingress within the current project or namespace. For more information, see [Adding Ingress]({{<baseurl>}}/rancher/latest/en/k8s-in-rancher/load-balancers-and-ingress/ingress/).
