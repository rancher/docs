...
You can create a cluster from an RKE template that you created, or from a template that has been [shared with you.]({{<baseurl>}}/rancher/latest/en/admin-settings/rke-templates/template-access-and-sharing)
...
To add a cluster [hosted by an infrastructure provider]({{<baseurl>}}/rancher/latest/en/cluster-provisioning/rke-clusters) using an RKE template, use these steps:
...
- If the setting allows a user override, you can update these settings in the cluster by [editing the cluster.]({{<baseurl>}}/rancher/latest/en/cluster-admin/editing-clusters/)
...
- New clusters can be [created from the new template.]({{<baseurl>}}/rancher/latest/en/admin-settings/rke-templates/applying-templates/#creating-a-cluster-from-an-rke-template)