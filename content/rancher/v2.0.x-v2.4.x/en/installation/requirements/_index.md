...
> It is important to note that if you install Rancher on a Kubernetes cluster, requirements are different from the [node requirements for downstream user clusters,]({{<baseurl>}}/rancher/v2.0.x-v2.4.x/en/cluster-provisioning/node-requirements/) which will run your apps and services.
...
For a list of best practices that we recommend for running the Rancher server in production, refer to the [best practices section.]({{<baseurl>}}/rancher/v2.0.x-v2.4.x/en/best-practices/deployment-types/)
...
Some distributions of Linux may have default firewall rules that block communication with Helm. This [how-to guide]({{<baseurl>}}/rancher/v2.0.x-v2.4.x/en/installation/options/firewall) shows how to check the default firewall rules for Oracle Linux and how to open the ports with `firewalld` if necessary.

If you plan to run Rancher on ARM64, see [Running on ARM64 (Experimental).]({{<baseurl>}}/rancher/v2.0.x-v2.4.x/en/installation/options/arm64-platform/)

### Installing Docker

Docker can be installed by following the steps in the official [Docker documentation.](https://docs.docker.com/) Rancher also provides [scripts]({{<baseurl>}}/rancher/v2.0.x-v2.4.x/en/installation/requirements/installing-docker) to install Docker with one command.
...
These requirements apply to each host in an [RKE Kubernetes cluster where the Rancher server is installed.]({{<baseurl>}}/rancher/v2.0.x-v2.4.x/en/installation/k8s-install/)
...
These requirements apply to each host in a [K3s Kubernetes cluster where the Rancher server is installed.]({{<baseurl>}}/rancher/v2.0.x-v2.4.x/en/installation/k8s-install/)
...
These requirements apply to a host with a [single-node]({{<baseurl>}}/rancher/v2.0.x-v2.4.x/en/installation/other-installation-methods/single-node-docker) installation of Rancher.
...
The following diagram depicts the ports that are opened for each [cluster type]({{<baseurl>}}/rancher/v2.0.x-v2.4.x/en/cluster-provisioning).
...
**Note** Rancher nodes may also require additional outbound access for any external [authentication provider]({{<baseurl>}}/rancher/v2.0.x-v2.4.x/en/admin-settings/authentication/) which is configured (LDAP for example).
