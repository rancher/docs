...
When you use Rancher to [launch nodes for a cluster]({{<baseurl>}}/rancher/v2.0.x-v2.4.x/en/cluster-provisioning/#cluster-creation-in-rancher), resources (containers/virtual network interfaces) and configuration items (certificates/configuration files) are created.
...
After you initiate the removal of an [imported cluster]({{<baseurl>}}/rancher/v2.0.x-v2.4.x/en/cluster-provisioning/#import-existing-cluster) using the Rancher UI (or API), the following events occur.
