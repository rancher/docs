...
Monitoring also creates additional `ClusterRoles` that are not assigned to users by default but are created within the cluster.  They are not aggregated by default but can be bound to a namespace by deploying a RoleBinding that references it. To define a ClusterRoleBinding with `kubectl` instead of through Rancher, click [here]({{<baseurl>}}/rancher/v2.5/en/monitoring-alerting/rbac/#assigning-roles-and-clusterroles-to-users-with-kubectl).
